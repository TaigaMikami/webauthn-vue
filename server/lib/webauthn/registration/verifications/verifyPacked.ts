...
import convertCOSEtoPKCS, { COSEALGHASH, COSECRV, COSEKEYS, COSEKTY, COSERSASCHEME } from "../../helpers/convertCOSEtoPKCS";
  ...
  if (x5c) {
    ...
    if (statement) {
      ...
      try {
        ...
        throw new Error(`${(err as any).message} (Packed|Full)`);
      }
    } else {
      try {
