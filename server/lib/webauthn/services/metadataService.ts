...
import { MDSJWTHeader, MDSJWTPayload, MetadataBLOBPayloadEntry, MetadataStatement } from "../metadata/mdsTypes";
   ...
   */
   async initialize(
    ...
    if (statements?.length) {
      ...
      console.log(`Cached ${statementsAdded} local statements`);
    }
    ...
    if (mdsServers?.length) {
      ...
      for (const url of mdsServers) {
        try {
          ...
          console.log(`Could not download BLOB from ${url}:`, err);
          numServers -= 1;
        }
      }
      ...
      console.log(`Cached ${cacheDiff} statements from ${numServers} metadata server(s)`);
    }
   ...
   */
   private setState(newState: SERVICE_STATE) {
    ...
    if (newState === SERVICE_STATE.DISABLED) {
      console.log('MetadataService is DISABLED');
    } else if (newState === SERVICE_STATE.REFRESHING) {
      console.log('MetadataService is REFRESHING');
    } else if (newState === SERVICE_STATE.READY) {
      console.log('MetadataService is READY');
    }
  }
}
