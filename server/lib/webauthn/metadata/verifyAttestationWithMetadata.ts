...
import { AlgSign, MetadataStatement } from "./mdsTypes";
 ...
 */
 export default async function verifyAttestationWithMetadata(
  ...
  try {
    ...
    throw new Error(`Could not validate certificate path with any metadata root certificates: ${(err as any).message}`);
  }
