...
export function identifyRegistrationError({
  ...
  if (error.name === "AbortError") {
    ...
    return new WebAuthnError(
      "The authenticator was previously registered (InvalidStateError)"
    );
    ...
    if (!isValidDomain(effectiveDomain)) {
      ...
      return new WebAuthnError(
        `${window.location.hostname} is an invalid domain (SecurityError)`
      );
    ...
    if (publicKey.user.id.byteLength < 1 || publicKey.user.id.byteLength > 64) {
      ...
      return new WebAuthnError(
        "User ID was not between 1 and 64 characters (TypeError)"
      );
    }
